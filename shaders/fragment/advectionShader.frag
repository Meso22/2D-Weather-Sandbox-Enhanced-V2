...
void main()
{
  ...
  if (wall[DISTANCE] != 0) { // not wall
    ...
    // Backward semi-Lagrangian advection for scalars (combined into single bilerpWall calls)
    vec4 backBase  = bilerpWall(baseTex,  wallTex, fragCoord - velAtP);
    vec4 backWater = bilerpWall(waterTex, wallTex, fragCoord - velAtP);
    base[PRESSURE]    = backBase[PRESSURE];
    base[TEMPERATURE] = backBase[TEMPERATURE];
    water.xyw = backWater.xyw;

    water[PRECIPITATION] = bilerpWall(waterTex, wallTex, fragCoord - velAtP + vec2(0, 0.05))[PRECIPITATION];

    // MacCormack anti-diffusion: corrects half the bilinear truncation error by comparing
    // the backward trace to a forward trace from the same origin field. Result is clamped
    // to the [backward, original] interval to prevent oscillatory overshoot.
    vec4 fwdBase  = bilerpWall(baseTex,  wallTex, fragCoord + velAtP);
    vec4 fwdWater = bilerpWall(waterTex, wallTex, fragCoord + velAtP);
    vec4 waterX0Y0 = texture(waterTex, texCoord);

    float T_mc = base[TEMPERATURE] + 0.5 * (cellX0Y0[TEMPERATURE] - fwdBase[TEMPERATURE]);
    base[TEMPERATURE] = clamp(T_mc,
                              min(backBase[TEMPERATURE], cellX0Y0[TEMPERATURE]),
                              max(backBase[TEMPERATURE], cellX0Y0[TEMPERATURE]));

    float W_mc = water[TOTAL] + 0.5 * (waterX0Y0[TOTAL] - fwdWater[TOTAL]);
    water[TOTAL] = clamp(W_mc,
                         max(min(backWater[TOTAL], waterX0Y0[TOTAL]), 0.0),
                         max(backWater[TOTAL], waterX0Y0[TOTAL]));

    float C_mc = water[CLOUD] + 0.5 * (waterX0Y0[CLOUD] - fwdWater[CLOUD]);
    water[CLOUD] = clamp(C_mc,
                         max(min(backWater[CLOUD], waterX0Y0[CLOUD]), 0.0),
                         min(max(backWater[CLOUD], waterX0Y0[CLOUD]), water[TOTAL]));
