...
void main()
{
  ...
  float cosAngle = cos(sunAngle);

  // pow(x, 0.1) stays ~0.63 right up to x=0 then drops to 0 — near-discontinuous.
  // Multiplying by smoothstep tames that steep drop so the whole sky fades smoothly.
  float c = max(cosAngle, 0.0);
  light = pow(c, 0.1) * smoothstep(0.0, 0.08, c);
  ambientLight = vec3(0.0); // shadowed ambient texture ignored for sky background
  ...
  // Sunrise/sunset: peak when sun is near horizon, fade for daytime and deep night
  float ssf      = smoothstep(-0.10, 0.0, cosAngle) * (1.0 - smoothstep(0.0, 0.30, cosAngle));
  float altFade  = 1.0 - smoothstep(0.0, 1.0, texCoord.y);
  float altBlend = smoothstep(0.0, 0.7, texCoord.y);
  vec3  sunriseColor = mix(vec3(1.0, 0.38, 0.04), vec3(0.80, 0.10, 0.40), altBlend);
  // Warm tint is restricted to lower sky via altFade
  mixedCol = mix(mixedCol, sunriseColor, ssf * altFade * 0.70);

  // Daytime atmospheric haze: pale blue-white near the horizon, fades with altitude
  // dayFactor is 0 during sunrise/sunset window so the two effects don't overlap
  float dayFactor = smoothstep(0.10, 0.35, cosAngle);
  float hazeFade  = 1.0 - smoothstep(0.0, 0.4, texCoord.y);
  mixedCol = mix(mixedCol, vec3(0.82, 0.91, 1.0), dayFactor * hazeFade * 0.50);
  ...
  // twilightGlow has NO altFade so it stabilises the whole sky's brightness —
  // including the upper sky — preventing the black pop at the horizon crossing.
  float twilightGlow = ssf * 0.45 * (1.0 - light);

  // Moonlight ambient: fades in as sun dips below horizon so clouds remain barely
  // visible against the night sky. Peaks at ~0.04 when fully dark.
  float nightAmbient = 0.04 * smoothstep(0.0, -0.20, cosAngle);

  vec3 finalColor = mixedCol * (light + minShadowLight + airplaneOnLight + twilightGlow + nightAmbient);
  ...
  fragmentColor = vec4(finalColor * skyBrightness, 1.0);
}