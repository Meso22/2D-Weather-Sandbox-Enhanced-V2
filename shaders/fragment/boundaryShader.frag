...
void main()
{
  ...
  if (wall[DISTANCE] != 0) {                         // is fluid, not wall
    ...
    vec4 waterAbove = texture(waterTex, texCoordX0Yp);
...
#define gravMult 0.0001 // 0.0001 0.0005
    ...
    // Virtual temperature: water vapor (MW=18) is lighter than dry air (MW≈29), adding
    // positive buoyancy. Tv = T*(1 + 0.608*r) where r=q_v/rho_air, rho_air≈1200 g/m³.
    // Interpolated across the cell boundary (VY is defined at the top of the cell).
    float qv     = max(water[TOTAL]    - water[CLOUD],    0.0);
    // Guard: wall cells carry marker values (1001/1002) in water[TOTAL], not actual vapor.
    float qv_up  = (wallX0Yp[DISTANCE] != 0) ? max(waterAbove[TOTAL] - waterAbove[CLOUD], 0.0) : qv;
    float avgT   = (base[TEMPERATURE] + baseX0Yp[TEMPERATURE]) * 0.5;
    gravityForce += avgT * (qv + qv_up) * 0.5 * (0.608 / 1200.0) * gravMult;
...
#define surfaceWindSmootingDist 5
    ...
    if (wall[VERT_DISTANCE] <= 8) { // within height of buildings
      ...
      case WALLTYPE_LAND:
        if (wall[VERT_DISTANCE] <= wallVerticalInfluence) {
          ...
          base[TEMPERATURE] -= evaporation * evapHeat * 0.0;                                // evaporative cooling (half the real value, to prevent boring non convective conditions)
    ...
    if (wall[VERT_DISTANCE] < 0) {                                         // below surface
      ...
      case WALLTYPE_LAND:                                                                                          // no break,can also be fire or urban:
        water[SOIL_MOISTURE] = clamp(water[SOIL_MOISTURE] + precipDeposition[RAIN_DEPOSITION] * 0.0, 0.0, 1000.0); // rain accumulation
        ...
        float evaporation = calcEvaporation(realTempAboveSurface, waterAboveSurface[TOTAL], float(wall[VEGETATION]), water[SOIL_MOISTURE]) * 0.0;
        ...
        if (int(iterNum) % 100 == 0) { // snow and soil moisture smoothing
          ...
          int vegetationGrowthRate = int(water[SOIL_MOISTURE] * sqrt(lightAboveSurface[SUNLIGHT]) * 0.00000000000000000000000000000001);
      ...
      case WALLTYPE_WATER:
        ...
        if (dynamicWaterTemperature >= 1.0 && mod(iterNum, waterTempUpdateInterval) < 0.5) {
          ...
          netWaterHeating -=
            max((maxWater(base[TEMPERATURE]) - waterX0Yp[TOTAL]) * waterEvaporation, 0.) * evapHeat * 0.0; // evaporative cooling (half the real value, to prevent boring non convective conditions)
