...
const guiControls_default = {
  ...
  timeOfDay : 9.9,
  latitude : -30.0,
  month : 10.00, // Northern hemisphere summer solstice
  sunAngle : 9.9,
  dayNightCycle : true,
...
async function mainScript(initialBaseTex, initialWaterTex, initialWallTex, initialRainDrops)
{
  ...
  // Radar dBZ cursor readout overlay
  const radarDbzDiv = document.createElement('div');
  radarDbzDiv.style.cssText = 'position:fixed;pointer-events:none;display:none;font-family:monospace;font-size:13px;font-weight:bold;background:rgba(0,0,0,0.72);padding:2px 8px;border-radius:3px;white-space:nowrap;z-index:100;';
  document.body.appendChild(radarDbzDiv);
  const radarDbzPixelBuf = new Float32Array(4); // reused every frame — avoids per-frame allocation
  ...
  function setupDatGui(strGuiControls)
  {
    ...
    display_folder.add(guiControls, 'contrast', 0.0, 3.0, 0.01)
      .onChange(function() {
        gl.useProgram(postProcessingProgram);
    ...
    display_folder.add(guiControls, 'skyBrightness', 0.0, 3.0, 0.01)
      .onChange(function() {
        gl.useProgram(skyBackgroundDisplayProgram);
        gl.uniform1f(gl.getUniformLocation(skyBackgroundDisplayProgram, 'skyBrightness'), guiControls.skyBrightness);
      })
      .name('Sky Brightness');
    ...
    display_folder.add(guiControls, 'radarPixelSize', 1.0, 16.0, 1.0).name('Radar Pixel Size');
    display_folder.add(guiControls, 'showRadarDbz').name('Show Radar dBZ').listen();
    display_folder.add(guiControls, 'radarOverlay').name('Radar Overlay (Realistic)').listen();
    display_folder.add(guiControls, 'radarOverlayOpacity', 0.1, 1.0, 0.01).name('Radar Overlay Opacity');
  ...
  const radarPrecipDisplayVertexShader = await loadShader('radarPrecipDisplayShader.vert');
  const radarPrecipDisplayShader = await loadShader('radarPrecipDisplayShader.frag');
  ...
  const radarPrecipDisplayProgram = createProgram(radarPrecipDisplayVertexShader, radarPrecipDisplayShader);
  ...
  */
  ...
  gl.uniform1f(gl.getUniformLocation(radarDisplayProgram, 'isOverlay'), 0.0);
  gl.uniform1f(gl.getUniformLocation(radarDisplayProgram, 'overlayOpacity'), 0.82);

  gl.useProgram(radarPrecipDisplayProgram);
  gl.uniform2f(gl.getUniformLocation(radarPrecipDisplayProgram, 'resolution'), sim_res_x, sim_res_y);
  gl.uniform2f(gl.getUniformLocation(radarPrecipDisplayProgram, 'texelSize'), texelSizeX, texelSizeY);
  ...
  { // Runs for every frame
    ...
    if (dropletFollowID >= 0) {
      ...
      let totalMass = dropletInfo[2] + dropletInfo[3];
      let diameterMm = 2.0 * Math.pow(totalMass, 1.0 / 3.0);
      ctx.fillText('Size  : ' + diameterMm.toFixed(2) + ' mm', 0, 60);
    }
    ...
    if (guiControls.displayMode == 'DISP_REAL') {
      ...
      if (guiControls.radarOverlay) {
        ...
        gl.uniform1f(gl.getUniformLocation(radarDisplayProgram, 'radarPixelSize'), guiControls.radarPixelSize);
        gl.uniform1f(gl.getUniformLocation(radarDisplayProgram, 'isOverlay'), 1.0);
        gl.uniform1f(gl.getUniformLocation(radarDisplayProgram, 'overlayOpacity'), guiControls.radarOverlayOpacity);
        gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
        gl.uniform1f(gl.getUniformLocation(radarDisplayProgram, 'isOverlay'), 0.0);
        gl.disable(gl.BLEND);
      }
      ...
      // Bind water and wall textures for the background field shader
      gl.activeTexture(gl.TEXTURE1);
      gl.bindTexture(gl.TEXTURE_2D, waterTexture_1);
      gl.activeTexture(gl.TEXTURE2);
      gl.bindTexture(gl.TEXTURE_2D, wallTexture_1);

      // Draw radar background: smooth precipitation field + terrain
      gl.useProgram(radarDisplayProgram);
      gl.uniform2f(gl.getUniformLocation(radarDisplayProgram, 'aspectRatios'), sim_aspect, canvas_aspect);
      gl.uniform3f(gl.getUniformLocation(radarDisplayProgram, 'view'), cam.curXpos, cam.curYpos, cam.curZoom);
      gl.uniform4f(gl.getUniformLocation(radarDisplayProgram, 'cursor'), mouseXinSim, mouseYinSim, guiControls.brushSize * 0.5, cursorType);
    ...
    // Radar dBZ cursor readout
    if ((guiControls.displayMode == 'DISP_RADAR' || (guiControls.displayMode == 'DISP_REAL' && guiControls.radarOverlay)) && guiControls.showRadarDbz) {
      const simXpos = Math.floor(Math.abs(mod(mouseXinSim * sim_res_x, sim_res_x)));
      const simYpos = Math.min(Math.max(Math.floor(mouseYinSim * sim_res_y), 0), sim_res_y - 1);

      gl.bindFramebuffer(gl.FRAMEBUFFER, frameBuff_1);
      gl.readBuffer(gl.COLOR_ATTACHMENT1); // water texture
      gl.readPixels(simXpos, simYpos, 1, 1, gl.RGBA, gl.FLOAT, radarDbzPixelBuf);
      gl.bindFramebuffer(gl.FRAMEBUFFER, null);

      const precip = radarDbzPixelBuf[2]; // PRECIPITATION channel
      const dbz = 10.0 * Math.log(precip * 5000.0 + 1.0) / Math.log(10.0);

      const rect = canvas.getBoundingClientRect();
      radarDbzDiv.style.left = (rect.left + mouseX + 16) + 'px';
      radarDbzDiv.style.top  = (rect.top  + mouseY - 20) + 'px';
      radarDbzDiv.style.display = 'block';

      if (dbz >= 5.0) {
        radarDbzDiv.style.color = radarDbzColor(dbz);
        radarDbzDiv.textContent = dbz.toFixed(1) + ' dBZ';
      } else {
        radarDbzDiv.style.color = '#777';
        radarDbzDiv.textContent = 'ND';
      }
    } else {
      radarDbzDiv.style.display = 'none';
    }
  ...
  // Returns a CSS hex colour matching the NEXRAD reflectivity scale
  function radarDbzColor(dbz) {
    if (dbz < 10) return '#01EDEE';
    if (dbz < 15) return '#019FE0';
    if (dbz < 20) return '#0300F4';
    if (dbz < 25) return '#02FD02';
    if (dbz < 30) return '#01C501';
    if (dbz < 35) return '#008E00';
    if (dbz < 40) return '#FDF802';
    if (dbz < 45) return '#E5BC00';
    if (dbz < 50) return '#FD9500';
    if (dbz < 55) return '#FD0000';
    if (dbz < 60) return '#D40000';
    if (dbz < 65) return '#F800FD';
    if (dbz < 70) return '#9854C6';
    return '#ffffff';
  }
  ...
  function updateSunlight(deltaT_hours)
  {
    ...
    minShadowLight = map_range_C(Math.abs(solarZenithAngleDeg), 100.0, 85.0, 0.010, 0.060); // decrease until the sun goes 10 deg below the horizon
